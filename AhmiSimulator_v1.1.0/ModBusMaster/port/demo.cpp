...
USHORT   usMStringBuf[STRING_NSTRINGS][STRING_MAXLEN];
...
eMBErrorCode
eMBStringCB( UCHAR * pucRegBuffer, USHORT usAddress, USHORT usNRegs ,eMBRegisterMode eMode)
{
  
 eMBErrorCode    eStatus = MB_ENOERR;
    int             iRegIndex = 0;
	USHORT             i;
    if( ( usAddress >= STRING_START )
        && ( usAddress + usNRegs <= STRING_START + STRING_NSTRINGS ) )
    {
		switch (eMode)
		{
		 case MB_REG_READ:
				*pucRegBuffer++ = STRING_MAXLEN -2;
				i = STRING_MAXLEN - 2;
				while( i > 0 )
				{
					*pucRegBuffer++ = (usMStringBuf[usAddress - 1][iRegIndex] + i);
                 ...
                 eMBRegisterMode eMode )
{
        ...
        ( usAddress + usNRegs <= REG_HOLDING_START + REG_HOLDING_NREGS ) )
    {
        ...
        case MB_REG_WRITE:
			...
			{
					if( (TagUpdatePtr + i) ->mRegID == iRegIndex  && TagUpdatePtr[i].mTagRW == TAGREAD) // 当前tag需要根据读取的数据更新
					(TagPtr + TagUpdatePtr[i].mTagID)->setValue((u32)usRegHoldingBuf[iRegIndex]);
			}
                iRegIndex++;
                usNRegs--;
            }
