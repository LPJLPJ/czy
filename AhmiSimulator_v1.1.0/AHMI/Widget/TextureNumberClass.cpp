...
//-----------------------------
// 函数名： TextureNumberClass::initWidget
...
//图层数字控件
//显示N个字符，需要N张纹理。
//支持小数（一个小数点）,N+1张纹理。支持符号的显示，正数不显示加号，负数显示减号
//只支持tag输入类型。
//WidgetAttr标识：
...
//	gPagePtr[WorkingPageID].pBasicTextureList[p_wptr->StartNumofTex].mTexAttr |= TEXTURE_USING_WIDGET_BOX; //puremask第一层必须使用控件的包围盒
	...
	for(i=0;i<numofNumber;i++)
	{
		...
		if(i == pointPosLeft)
		{
			if(overflow == 1)
				...
				texturePtr->FocusedSlice = MPOINT;//显示小数点
			}
			texturePtr++;
			offsetpoint = i;
			i++;
		...
		tens =tens /10;
		if(overflow == 1)
			texturePtr->FocusedSlice = SPACE;    //等于13 显示空字符，不显示
		else
		...
		for(i=0;i<numofNumber;i++)
		{
			...
			if(i == pointPosLeft)
			{
				if(overflow == 1)
					...
					nextTexturePtr->FocusedSlice = MPOINT;//显示小数点
				nextTexturePtr++;
				offsetpoint = i;
				i++;
			}
	...
	for(i=0;i<numofNumber;i++)
	{
		...
		if(texturePtr[i].FocusedSlice == SPACE) //初始位置不在页面内，但动画后在页面内需要显示 by Mr.z
		{
			texturePtr[i].mTexAttr &= (~DRAWING);
		}
		else 
	...
	if((p_wptr->NumOfLine & 0xff) == 2) //with animation
	{
		...
		for(i=0;i<numofNumber;i++)
		{
			...
			//if( (nextTexturePtr[i].OffsetX + (widthOfFont ) ) < 0 || (nextTexturePtr[i].FocusedSlice == SPACE)  ) //修改在有动画的情况下，纹理不一定在屏幕上，会导致初始不在屏幕上的文字，执行动画结束后也不显示
			if(nextTexturePtr[i].FocusedSlice == SPACE) //初始位置不在页面内，但动画后在页面内需要显示 by Mr.z 
			{
				nextTexturePtr[i].mTexAttr &= (~DRAWING);
			}
			else 
