...
//	RefreshMsg refreshMsg;
	...
	//}
	////优化指针包围盒
	//TexturePtr[PointerPtr].renewTextureSourceBox();
	...
	//			TexturePtr[(PointerPtr - 1)%4 + 2 ].renewTextureSourceBox();
	//		}
	//	}
	//	else //逆时针旋转
	//	{
	...
	//			TexturePtr[PointerPtr + 1].renewTextureSourceBox();
	//		}
	//	}
	//}
	...
	//			TexturePtr[(PointerPtr - 1)%4 + 1 ].renewTextureSourceBox();
	//		}
	//	}
	//	else //逆时针旋转
	//	{
	...
	//	}
	//}
	//else if(MeterMode == 3)//双向模式
	//{
	//	Angle = Angle + (((s16)(p_wptr->MeterMinAngle)) << 4);
	//	if(Angle >=0)
	//	{
	//	for(PointerPtr = 1  ;PointerPtr<5;PointerPtr++)
	//		{
	...
	//				ScaleY = 512;
	//			}
	//			TexturePtr[(PointerPtr - 1)%4 + 2 ].RotateAngle = RotateAngle + pureMaskAdjustAngle -(((s16)(p_wptr->MeterMinAngle)) << 4);
	//			TexturePtr[(PointerPtr - 1)%4 + 2 ].ShearAngleX = ShearAngleX;
	//			TexturePtr[(PointerPtr - 1)%4 + 2 ].ShearAngleY = ShearAngleY;
	//			TexturePtr[(PointerPtr - 1)%4 + 2 ].ScalerX = ScaleX;
	//			TexturePtr[(PointerPtr - 1)%4 + 2 ].ScalerY = ScaleY;
	...
	//				ScaleY = 512;
	//			}
	//			TexturePtr[PointerPtr + 1].RotateAngle = RotateAngle + pureMaskAdjustAngle - (((s16)(p_wptr->MeterMinAngle)) << 4) ;
	//			TexturePtr[PointerPtr + 1].ShearAngleX =ShearAngleX;//ShearAngle;
	//			TexturePtr[PointerPtr + 1].ShearAngleY =ShearAngleY;//ShearAngle;
	//			TexturePtr[PointerPtr + 1].ScalerX = ScaleX;
	//			TexturePtr[PointerPtr + 1].ScalerY = ScaleY;
	//			TexturePtr[PointerPtr + 1].renewTextureSourceBox();
	//		}
	...
	if(rotatingDir)//顺时针
		{
			...
			//调整指针纹理的放缩量 by Mr.z
			if(p_wptr->WidgetWidth != p_wptr->WidgetHeight){
				TexturePtr[PointerPtr].ScalerX = (p_wptr->WidgetWidth << 9 )/p_wptr->WidgetHeight;
			}
			Angle = TexturePtr[PointerPtr].RotateAngle - ((s16)(p_wptr->MeterMinAngle) << 4) - startAngle;// + (45*16);//////指针角度和起始角度之间相对角度  （小小火车侠）
		}
	else //逆时针
