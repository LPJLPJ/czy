...
//时间日期控制函数
//仅仅显示数字，不包括背景纹理。
//数字通过tag传入，每4bit的tag表示单个数字。
//时间的显示方式为hour:minite:second,对应tag[23:16]:tag[15:8]:tag[7:0]
//一共8个字符，9张纹理（一张背景alpha纹理）
//日期的显示方式为year/month/day,对应tag[31:8]/tag[15:8]/tag[7:0]
//一共10个字符，11张纹理
//WidgetAttr标识：
//15-7:保留
//5   :显示模式。0表示显示时间，1表示显示日期。
//4-0 :控件类型
//**************************************
	...
	if(ActionPtr->mInputType == ACTION_TAG_SET_VALUE || u8_pageRefresh == 1)
	{
		...
		if(displayMode == CLOCK_HOUR_MINUTE_MODE) //时分模式
		{
			for(i = 0; i < 4; i++)
			{
				...
				code = (divValue & 0xf) + 0x30 - 0x20/*基础偏移量*/;
				texturePtr->FocusedSlice = code;
				texturePtr++;
			}
		}
	...
	else if(ActionPtr->mInputType == ACTION_KEYBOARD_PRE) //向左按钮
	{
		if(modifyEn == 0) //非修改模式
		{
			if(displayMode == CLOCK_HOUR_MINUTE_MODE) //时分模式
			{
				...
				{
					texturePtr[numOfChar + 1].mTexAttr |= (DRAWING); //颜色按钮显示
