...
#define TIMEOUT_READ_TOTAL_CONSTANT  0 // 写超时为秒 
...
// 打开串口 
BOOL CMySerialPort::Open ( int nPort, int nBaud ) 
{ 
...
BOOL CMySerialPort::Close ( void ) { 
    ...
    return ( TRUE ); } 
// 向串口写入数据，类内部使用 
BOOL CMySerialPort::WriteCommByte ( unsigned char ucByte ) { 
    BOOL bWriteStat; 
...
// 向串口发送数据 
int CMySerialPort::SendData ( const char *buffer, int size ) 
{ 
...
int CMySerialPort::ReadDataWaiting ( void ) 
{
    if ( !m_bOpened || m_hIDComDev == NULL ) return ( 0 );  
    DWORD dwErrorFlags;
	COMSTAT ComStat;  
...
// 读取来自串口的数据 
int CMySerialPort::ReadData ( void *buffer, int limit ) 
{  
    if ( !m_bOpened || m_hIDComDev == NULL ) return ( 0 );  
    BOOL bReadStatus; 
