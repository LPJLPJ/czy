...
extern void CommSendChar(char data);
...
/*
 Create as a high priority thread, this function receives message from real
 Uart receiving funciton in MFC funtions. 
*/
static DWORD WINAPI prvSImulatedPeripheralUart( LPVOID lpParameter );
/*
 Create as a high priority thread, this function receives message from Mainframe
 Mouse click.
*/
static DWORD WINAPI prvSimulatedPeripheralMouse( LPVOID lpParameter );
...
static uint32_t prvProcessUartInterrupt( void );
static uint32_t prvProcessMouseInterrupt( void );
static uint32_t prvProcessUartSendInterrupt( void );
/*
...
/*
  An event used to inform the uart data comming.
*/
void *pvUartEvent = NULL;
/*
  Uart receive buffer.
*/
unsigned char Revcbuf[256];
int  RevcLength;

////****************************************
//// STM32 UART receive simulate buffer
////****************************************
//unsigned char STM32RevcQueue[512] = {0};
//int  STM32RevcQueueTail = 0;
//int  STM32RevcQueueHead = 0;
//int  STM32RevcQueueEmpty = 1;
//int  STM32RevcQueueFull = 0;


/*
  An event used to inform the Mouse click.
*/
void *pvMouseEvent = NULL;
/*
  Mouse click data.
*/
PIDState MouseClickData;
...
static DWORD WINAPI prvSImulatedPeripheralUart( LPVOID lpParameter )
{
	for(;;)
	{
		WaitForSingleObject( pvUartEvent, INFINITE );
		ulPendingInterrupts |= ( 1 << portINTERRUPT_UART );
		SetEvent( pvInterruptEvent );
	}
}
...
static DWORD WINAPI prvSimulatedPeripheralMouse(LPVOID lpParameter )
{
	for(;;)
	{
		WaitForSingleObject( pvMouseEvent, INFINITE );
		ulPendingInterrupts |= ( 1 << portINTERRUPT_MOUSE );
		SetEvent( pvInterruptEvent );
	}
}
...
{
	CommSendChar(data);
	ulPendingInterrupts |= ( 1 << portINTERRUPT_UARTSEND );
	SetEvent( pvInterruptEvent );
}
...
xThreadState *pxThreadState;
	...
	vPortSetInterruptHandler( portINTERRUPT_UART, prvProcessUartInterrupt );
	vPortSetInterruptHandler( portINTERRUPT_MOUSE, prvProcessMouseInterrupt );
	vPortSetInterruptHandler( portINTERRUPT_UARTSEND, prvProcessUartSendInterrupt );
	
	/* Create the events and mutexes that are used to synchronise all the
	threads. */
	pvInterruptEventMutex = CreateMutex( NULL, FALSE, NULL );
	pvInterruptEvent = CreateEvent( NULL, FALSE, FALSE, NULL );
	pvUartEvent = CreateEvent(NULL,FALSE,FALSE,NULL);
	pvMouseEvent = CreateEvent(NULL,FALSE,FALSE,NULL);
	...
	if( lSuccess == pdPASS )
	{
		...
		pvHandle = CreateThread( NULL, 0, prvSImulatedPeripheralUart, NULL, 0, NULL );
		if( pvHandle != NULL )
		{
			SetThreadPriority( pvHandle, THREAD_PRIORITY_BELOW_NORMAL );
			SetThreadPriorityBoost( pvHandle, TRUE );
		...
		pvHandle = CreateThread( NULL, 0, prvSimulatedPeripheralMouse, NULL, 0, NULL );
		if( pvHandle != NULL )
		{
			SetThreadPriority( pvHandle, THREAD_PRIORITY_BELOW_NORMAL );
			SetThreadPriorityBoost( pvHandle, TRUE );
...
static uint32_t prvProcessUartInterrupt()
{
	int i=0;
	while(i<RevcLength)
	{
		...
		/*xQueueSendToBack(UartDataRecvQueue,&Revcbuf[i],NULL);*/
		receivedata = Revcbuf[i];
...
#ifdef USE_UART
		xQueueSendToBack(UartDataRecvQueue,&Revcbuf[i],portMAX_DELAY);
		i++;
#endif
	}
#ifdef USE_MODBUS_MASTER
...
static uint32_t prvProcessMouseInterrupt()
{
#ifdef DEBUG
	char showdata[100];

	sprintf_s(showdata,100,"Receive Mouse %s at <%d,%d>.\r\n",MouseClickData.press?"Up":"Down",MouseClickData.x,MouseClickData.y);
	ERROR_PRINT(showdata);
#endif
	xQueueSendToFrontFromISR(MouseQueue,(void*)&MouseClickData,NULL);
	return pdTRUE;
}
