...
//将数据拷贝至特定位置
BaseType_t arthasCopyDataToQueueInPos(QueueHandle_t xQueue, const void *pvItemToQueue, const BaseType_t xLocation )
{
BaseType_t xReturn = pdFALSE;
int8_t *pos;
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	...
	{
		pos = ( pxQueue->u.pcReadFrom ) + ( 1 + xLocation ) * ( pxQueue->uxItemSize ) ;
		( void ) memcpy( ( void * ) pos, pvItemToQueue, ( size_t ) pxQueue->uxItemSize ); /*lint !e961 !e418 MISRA exception as the casts are only redundant for some ports, plus previous logic ensures a null pointer can only be passed to memcpy() if the copy size is 0. */
		pxQueue->pcWriteTo += pxQueue->uxItemSize;
		if( pxQueue->pcWriteTo >= pxQueue->pcTail ) /*lint !e946 MISRA exception justified as comparison of pointers is the cleanest solution. */
		{
			pxQueue->pcWriteTo = pxQueue->pcHead;
...
//从特定位置读取数据
void arthasReadDataFromQueue( QueueHandle_t xQueue, void * const pvBuffer, const BaseType_t xLocation )
{
	Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	int8_t *pos;
	if( pxQueue->uxItemSize != ( UBaseType_t ) 0 )
	{
		pos = ( pxQueue->u.pcReadFrom ) + (1 + xLocation) * pxQueue->uxItemSize;
		( void ) memcpy( ( void * ) pvBuffer, ( void * ) pos, ( size_t ) pxQueue->uxItemSize ); /*lint !e961 !e418 MISRA exception as the casts are only redundant for some ports.  Also previous logic ensures a null pointer can only be passed to memcpy() when the count is 0. */
	}
}
/*-----------------------------------------------------------*/
